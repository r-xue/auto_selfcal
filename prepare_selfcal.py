...
def prepare_selfcal(vislist,
    ...
    listdict, bands, band_properties, scantimesdict, scanfieldsdict, scannfieldsdict, scanstartsdict, scanendsdict, integrationsdict, \
        integrationtimesdict, spwslist_dict, spwstring_dict, spwsarray_dict, mosaic_field, gaincalibrator_dict, spectral_scan, spws_set = importdata(
          ...
          for vis in vislist:
              ...
              for i in range(len(mosaic_field[band][vis][target]['field_ids'])):
                  ...
                  if not found:
                      ...
                      selfcal_library[target][band]['sub-fields-fid_map'][vis][len(
                          all_phasecenters)-1] = mosaic_field[band][vis][target]['field_ids'][i]
    ...
    for target in all_targets:
     for band in selfcal_library[target].keys():
       ...
       for vis in vislist:
          ...
          selfcal_library[target][band][vis]['spws_orig'] = band_properties_orig[vis.replace(
              '.selfcal', '')][band]['spwstring']
          selfcal_library[target][band][vis]['spwsarray_orig'] = band_properties_orig[vis.replace(
              '.selfcal', '')][band]['spwarray']
          selfcal_library[target][band][vis]['spwlist_orig'] = band_properties_orig[vis.replace(
              '.selfcal', '')][band]['spwarray'].tolist()
       ...
       for fid in selfcal_library[target][band]['sub-fields']:
           ...
           selfcal_library[target][band][fid]['vislist'] = [
               vis for vis in vislist if fid in selfcal_library[target][band]['sub-fields-fid_map'][vis]]
           selfcal_library[target][band][fid]['obstype'] = 'single-point'
           allscantimes = np.array([])
           allscannfields = np.array([])
           for vis in selfcal_library[target][band][fid]['vislist']:
              good = np.array([str(selfcal_library[target][band]['sub-fields-fid_map'][vis][fid])
                              in scan_fields for scan_fields in scanfieldsdict[band][vis][target]])
              ...
              allscantimes = np.append(
                  allscantimes, scantimesdict[band][vis][target][good]/scannfieldsdict[band][vis][target][good])
              ...
              selfcal_library[target][band][fid][vis]['spws_orig'] = band_properties_orig[vis.replace(
                  '.selfcal', '')][band]['spwstring']
              selfcal_library[target][band][fid][vis]['spwsarray_orig'] = band_properties_orig[vis.replace(
                  '.selfcal', '')][band]['spwarray']
              selfcal_library[target][band][fid][vis]['spwlist_orig'] = band_properties_orig[vis.replace(
                  '.selfcal', '')][band]['spwarray'].tolist()
        ...
        for band in selfcal_library[target]:
           selfcal_library[target][band]['cellsize'], selfcal_library[target][band]['imsize'], selfcal_library[target][band]['nterms'], \
               ...
               get_image_parameters(selfcal_library[target][band]['vislist'], telescope, target,
                                    dict(zip(vislist, [mosaic_field[band][vis][target]['field_ids']
                                         for vis in vislist])), band,
    ...
    for target in all_targets:
       ...
       for band in selfcal_library[target]:
          ...
          if band in selfcal_library[target]:
             ...
             selfcal_plan[target][band]['applycal_mode'] = [
                 apply_cal_mode_default]*len(selfcal_plan[target][band]['solints'])
