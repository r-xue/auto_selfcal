    ...
    for band in selfcal_library[target]:
        ...
        for fid in selfcal_library[target][band]['sub-fields']:
            ...
            mosaic_dirty_SNR[fid], mosaic_dirty_RMS[fid], mosaic_dirty_NF_SNR[fid], mosaic_dirty_NF_RMS[fid] = get_image_stats(imagename,
                                                                                                                               imagename.replace(
                                                                                                                                   'image.tt0', 'mask'), '', selfcal_library[target][band][fid], (telescope != 'ACA' or aca_use_nfmask), 'dirty', 'dirty',
        ...
        for fid in selfcal_library[target][band]['sub-fields']:
            ...
            mosaic_initial_SNR[fid], mosaic_initial_RMS[fid], mosaic_initial_NF_SNR[fid], mosaic_initial_NF_RMS[fid] = get_image_stats(imagename,
                                                                                                                                       imagename.replace(
                                                                                                                                           'image.tt0', 'mask'), '', selfcal_library[target][band][fid], (telescope != 'ACA' or aca_use_nfmask), 'orig', 'orig',
                                                                                                                                       mosaic_sub_field=selfcal_library[target][band]["obstype"] == "mosaic")
        ...
        if selfcal_library[target][band]['nterms'] == 1:  # updated nterms if needed based on S/N and fracbw
            selfcal_library[target][band]['nterms'] = check_image_nterms(
                selfcal_library[target][band]['fracbw'], selfcal_library[target][band]['SNR_orig'])
        ...
        for band in selfcal_library[target].keys():
            ...
            for spw in selfcal_library[target][band]['spw_map']:
                ...
                dirty_SNR, dirty_RMS, dirty_per_spw_NF_SNR, dirty_per_spw_NF_RMS = get_image_stats(sani_target+'_'+band+'_'+str(spw) +
                                                                                                   '_dirty.image.tt0', sani_target+'_'+band+'_' +
                                                                                                   str(
                                                                                                       spw)+'_dirty.mask', '', selfcal_library[target][band],
                ...
                per_spw_SNR, per_spw_RMS, initial_per_spw_NF_SNR, initial_per_spw_NF_RMS = get_image_stats(sani_target+'_'+band+'_'+str(spw) +
                                                                                                           '_initial.image.tt0', sani_target+'_'+band+'_' +
                                                                                                           str(
                                                                                                               spw)+'_initial.mask', '', selfcal_library[target][band],
...
set_clean_thresholds(selfcal_library, selfcal_plan, dividing_factor=dividing_factor,
                     rel_thresh_scaling=rel_thresh_scaling, telescope=telescope)
    ...
    for band in bands:
        ...
        for target in selfcal_library.keys():
            ...
            if len(fallback_fields[band]) > 0:
                ...
                selfcal_plan[target][band]['gaincal_combine'] += [selfcal_plan[target][band]['gaincal_combine'][0], selfcal_plan[target][band]
                                                                  ['gaincal_combine'][1], selfcal_plan[target][band]['gaincal_combine'][1], selfcal_plan[target][band]['gaincal_combine'][1]]
                applycal_mode[band][target] += [applycal_mode[band][target][0], applycal_mode[band]
                                                [target][1], applycal_mode[band][target][1], applycal_mode[band][target][1]]
    ...
    for target in selfcal_library:
        ...
        for band in selfcal_library[target].keys():
            ...
            if 'gaintable_final' in selfcal_library[target][band][vislist[0]]:
                ...
                for vis in selfcal_library[target][band]['vislist']:
                    print('****************Applying ' +
                          str(selfcal_library[target][band][vis]['gaintable_final'])+' to '+target+' '+band+'*************')
...
for target in selfcal_library:
    ...
    for band in selfcal_library[target].keys():
        ...
        clean_threshold = min(selfcal_library[target][band]['clean_threshold_orig'],
        ...
        for fid in selfcal_library[target][band]['sub-fields']:
            ...
            mosaic_final_SNR[fid], mosaic_final_RMS[fid], mosaic_final_NF_SNR[fid], mosaic_final_NF_RMS[fid] = get_image_stats(imagename,
                                                                                                                               imagename.replace(
                                                                                                                                   'image.tt0', 'mask'), '', selfcal_library[target][band][fid], (telescope != 'ACA' or aca_use_nfmask), 'final', 'final',
                                                                                                                               mosaic_sub_field=selfcal_library[target][band]["obstype"] == "mosaic")
        ...
        for fid in selfcal_library[target][band]['sub-fields']:
            ...
            mosaic_final_SNR[fid], mosaic_final_RMS[fid], mosaic_final_NF_SNR[fid], mosaic_final_NF_RMS[fid] = get_image_stats(imagename+'_initial.image.tt0',
                                                                                                                               imagename +
                                                                                                                               '_final.mask', '', selfcal_library[target][band][fid], (
                                                                                                                                   telescope != 'ACA' or aca_use_nfmask), 'orig', 'orig',
                                                                                                                               mosaic_sub_field=selfcal_library[target][band]["obstype"] == "mosaic")
        ...
        for band in selfcal_library[target].keys():
            ...
            for spw in selfcal_library[target][band]['spw_map']:
                ...
                final_per_spw_SNR, final_per_spw_RMS, final_per_spw_NF_SNR, final_per_spw_NF_RMS = get_image_stats(
                    sani_target+'_'+band+'_'+str(spw)+'_final.image.tt0', sani_target +
                    '_'+band+'_'+str(spw)+'_final.mask',
                ...
                orig_final_per_spw_SNR, orig_final_per_spw_RMS, orig_final_per_spw_NF_SNR, orig_final_per_spw_NF_RMS = get_image_stats(
                    sani_target+'_'+band+'_'+str(spw)+'_initial.image.tt0', sani_target +
                    '_'+band+'_'+str(spw)+'_final.mask',
    ...
    for band in selfcal_library[target].keys():
        if selfcal_library[target][band]['SC_success']:
            for vis in selfcal_library[target][band]['vislist']:
                ...
                line = 'applycal(vis="'+vis.replace('.selfcal', '')+'",gaintable='+str(selfcal_library[target][band][vis]['gaintable_final'])+',interp='+str(selfcal_library[target][band][vis]['applycal_interpolate_final'])+', calwt=False,spwmap='+str(
                    selfcal_library[target][band][vis]['spwmap_final'])+', applymode="'+selfcal_library[target][band][vis]['applycal_mode_final']+'",field="'+target+'",spw="'+selfcal_library[target][band][vis]['spws_orig']+'")\n'
                applyCalOut.writelines(line)
                if apply_to_target_ms:
                    if os.path.exists(vis.replace('.selfcal', '')+".flagversions/flags.starting_flags"):
                        flagmanager(vis=vis.replace('.selfcal', ''), mode='restore',
                                    versionname='starting_flags', comment='Flag states at start of reduction')
                    else:
...
if casaversion[0] > 6 or (casaversion[0] == 6 and (casaversion[1] > 5 or (casaversion[1] == 5 and casaversion[2] >= 2))):
    if os.path.exists("cont.dat"):
        ...
        for target in selfcal_library:
            ...
            for band in selfcal_library[target].keys():
                ...
                spwvisref = get_spwnum_refvis(selfcal_library[target][band]['vislist'], target, contdotdat, dict(zip(selfcal_library[target][band]['vislist'], [
                                              selfcal_library[target][band][vis]['spwsarray'] for vis in selfcal_library[target][band]['vislist']])))
                for vis in selfcal_library[target][band]['vislist']:
                    msmd.open(vis)
                    field_num_array = msmd.fieldsforname(target)
                    msmd.close()
                    for fieldnum in field_num_array:
                        contsub_dict[vis][str(fieldnum)] = get_fitspw_dict(vis.replace('.selfcal', ''), target, selfcal_library[target]
                                                                           [band][vis]['spwsarray'], selfcal_library[target][band]['vislist'], spwvisref, contdotdat)
                        print(contsub_dict[vis][str(fieldnum)])
        print(contsub_dict)
        uvcontsubOut = open('uvcontsub_orig_MSes.py', 'w')
        for vis in selfcal_library[target][band]['vislist']:
            line = 'uvcontsub(vis="'+vis.replace('.selfcal', '')+'", spw="'+selfcal_library[target][band][vis]['spws']+'",fitspec='+str(
                contsub_dict[vis])+', outputvis="'+vis.replace('.selfcal', '').replace('.ms', '.contsub.ms')+'",datacolumn="corrected")\n'
        ...
        for target in selfcal_library:
            ...
            for band in selfcal_library[target].keys():
                ...
                spwvisref = get_spwnum_refvis(selfcal_library[target][band]['vislist'], target, contdotdat, dict(zip(selfcal_library[target][band]['vislist'], [
                                              selfcal_library[target][band][vis]['spwsarray'] for vis in selfcal_library[target][band]['vislist']])))
                for vis in selfcal_library[target][band]['vislist']:
                    contdot_dat_flagchannels_string = flagchannels_from_contdotdat(vis.replace(
                        '.selfcal', ''), target, selfcal_library[target][band][vis]['spwsarray'], selfcal_library[target][band]['vislist'], spwvisref, contdotdat, return_contfit_range=True)
                    line = 'uvcontsub(vis="'+vis.replace('.selfcal', '')+'", outputvis="'+sani_target+'_'+vis.replace('.selfcal', ''.replace('.ms', '.contsub.ms')) + \
                        '",field="'+target+'", spw="' + \
                        selfcal_library[target][band][vis]['spws']+'",fitspec="' + \
                        contdot_dat_flagchannels_string+'", combine="spw")\n'
                    uvcontsubOut.writelines(line)
        uvcontsubOut.close()
