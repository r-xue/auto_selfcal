...
from selfcal_helpers import check_image_nterms,checkmask,get_SNR_self_update,sanitize_string,tclean_wrapper,triage_calibrators,usermodel_wrapper
...
def run_selfcal(selfcal_library, selfcal_plan, target, band, telescope, n_ants,
    ...
    for iteration in range(len(selfcal_plan['solints'])):
        ...
        if mode == "selfcal" and selfcal_plan['solint_snr'][selfcal_plan['solints'][iteration]] < minsnr_to_proceed and np.all([selfcal_plan[fid]['solint_snr_per_field'][selfcal_plan['solints'][iteration]] < minsnr_to_proceed for fid in selfcal_library['sub-fields']]):
            print('*********** estimated SNR for solint='+selfcal_plan['solints'][iteration]+' too low, measured: '+str(
                selfcal_plan['solint_snr'][selfcal_plan['solints'][iteration]])+', Min SNR Required: '+str(minsnr_to_proceed)+' **************')
            # if a solution interval shorter than inf for phase-only SC has passed, attempt amplitude selfcal
            if iteration > 1 and selfcal_plan['solmode'][iteration] != 'ap' and do_amp_selfcal:
            ...
            for vis in vislist:
                ...
                selfcal_library[vis][solint]['clean_threshold'] = selfcal_library['nsigma'][iteration] * \
                    selfcal_library['RMS_NF_curr']
                ...
                for fid in np.intersect1d(selfcal_library['sub-fields-to-selfcal'], list(selfcal_library['sub-fields-fid_map'][vis].keys())):
                    ...
                    selfcal_library[fid][vis][solint]['clean_threshold'] = selfcal_library['nsigma'][iteration] * \
                        selfcal_library['RMS_NF_curr']
            ...
            for applymode in np.unique([selfcal_plan['applycal_mode'][iteration], 'calonly']):
                for vis in vislist:
                    ...
                    if mode == "cocal":
                        flagmanager(vis=vis, mode='restore', versionname='selfcal_starting_flags',
                                    comment='Flag states at start of the reduction')
                ...
                if selfcal_library['obstype'] == 'mosaic':
                    selfcal_library['sub-fields-to-gaincal'] = evaluate_subfields_to_gaincal(selfcal_library, target, band,
                                                                                             solint, iteration, selfcal_plan['solmode'], selfcal_plan[
                                                                                                 'solints'], selfcal_plan, minsnr_to_proceed,
                ...
                SNR, RMS, SNR_NF, RMS_NF = get_image_stats(sani_target+'_'+band+'_'+solint+'_'+str(iteration)+'.image.tt0',
                                                           sani_target+'_'+band+'_'+solint+'_' +
                                                           str(iteration)+'_post.mask', sani_target+'_' +
                                                           band+'_'+solint+'_'+str(iteration)+'.mask',
                ...
                for fid in selfcal_library['sub-fields-to-selfcal']:
                    ...
                    mosaic_SNR[fid], mosaic_RMS[fid], mosaic_SNR_NF[fid], mosaic_RMS_NF[fid] = get_image_stats(imagename+'.image.tt0',
                                                                                                               imagename+'_post.mask', imagename +
                                                                                                               '.mask', selfcal_library[fid], (
                                                                                                                   telescope != 'ACA' or aca_use_nfmask), solint,
                ...
                beamarea_post = selfcal_library[vislist[0]][solint]['Beam_major_post'] * \
                    selfcal_library[vislist[0]][solint]['Beam_minor_post']
                '''
             ...
             '''
                ...
                if selfcal_library['obstype'] == "mosaic" and not np.all(field_by_field_success):
                    field_by_field_success_dict = dict(
                        zip(selfcal_library['sub-fields-to-selfcal'], field_by_field_success))
                ...
                if mode == "cocal" and calculate_inf_EB_fb_anyways and solint == "inf_EB_fb" and selfcal_library["SC_success"]:
                    ...
                    for vis in vislist:
                        print('****************Applying ' +
                              str(selfcal_library[vis]['gaintable_final'])+' to '+target+' '+band+'*************')
            ...
            if np.any([selfcal_library[fid][selfcal_library[fid]['vislist'][0]][solint]['Pass'] == False for fid in
                ...
                if selfcal_library['final_solint'] == 'inf_EB' and selfcal_library['inf_EB_SNR_decrease']:
                    ...
                    for vis in vislist:
                        ...
                        selfcal_library[vis]['inf_EB']['Fail_Reason'] += ' with no successful solints later'
                ...
                for fid in np.intersect1d(selfcal_library['sub-fields'], list(selfcal_library['sub-fields-fid_map'][vis].keys())):
                    if (selfcal_library['final_solint'] == 'inf_EB' and selfcal_library['inf_EB_SNR_decrease']) or \
                        ...
                        for vis in vislist:
                            ...
                            # remove the success from inf_EB
                            selfcal_library[fid][vis]['inf_EB']['Fail_Reason'] += ' with no successful solints later'
            ...
            if selfcal_library[vislist[0]][solint]['Pass'] == True and \
                ...
                if mode == "selfcal" and (iteration < len(selfcal_plan['solints'])-1) and (selfcal_library[vis][solint]['SNR_post'] >
                    ...
                    for fid in selfcal_library['sub-fields-to-selfcal']:
                        print('Field '+str(fid)+' Was: ',
                              selfcal_plan[fid]['solint_snr_per_field'][selfcal_plan['solints'][iteration+1]])
                ...
                print('REASON: '+reason)
                # if a solution interval shorter than inf for phase-only SC has passed, attempt amplitude selfcal
                if mode == "selfcal" and iteration > 1 and selfcal_plan['solmode'][iteration] != 'ap' and do_amp_selfcal:
                    iterjump = selfcal_plan['solmode'].index('ap')
