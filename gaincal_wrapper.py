...
def gaincal_wrapper(selfcal_library, selfcal_plan, target, band, vis, solint, applymode, iteration, telescope,
    ...
    os.system('rm -rf '+sani_target+'_'+vis+'_'+band+'_'+solint+'_' +
              str(iteration)+'_'+selfcal_plan['solmode'][iteration]+'*.g')
    ##
    ...
    if selfcal_plan['solmode'][iteration] == 'p':
        if 'inf_EB' in solint:
            ...
            applycal_gaintable = [sani_target+'_'+vis+'_'+band+'_'+solint +
                                  '_'+str(iteration)+'_'+selfcal_plan['solmode'][iteration]+'.g']
        # elif selfcal_plan['solmode'][iteration]=='p':
           ...
           """
            ...
            applycal_gaintable = selfcal_library[vis][previous_solint]['gaintable'] + [sani_target +
                                                                                       '_'+vis+'_'+band+'_'+solint+'_'+str(iteration)+'_'+selfcal_plan['solmode'][iteration]+'.g']
        ...
        if mode == "cocal" and preapply_targets_own_inf_EB and "inf_fb" in solint and "inf" in selfcal_plan['solints']:
            ...
            ##
            destination_table = sani_target+'_'+vis+'_'+band+'_'+solint + \
                '_'+str(iteration)+'_'+selfcal_plan['solmode'][iteration]+'.g'
            for t in include_targets.split(","):
                if os.path.exists(sanitize_string(t)+'_'+vis+'_'+band+'_'+solint.replace('_fb1', '').replace('_fb2', '').replace('_fb3', '')+'_'+str(1)+'_'+selfcal_plan['solmode'][iteration] +
                    ...
                    table_name = sanitize_string(t)+'_'+vis+'_'+band+'_'+solint.replace('_fb1', '').replace(
                        '_fb2', '').replace('_fb3', '')+'_'+str(1)+'_'+selfcal_plan['solmode'][iteration]+'.g'
            ...
            for incl_scans, incl_targets in zip(include_scans, include_targets):
                ...
                for i in range(nspw_sets):  # run gaincal on each spw set to handle spectral scans
                    ...
                    gaincal_return_tmp = gaincal(vis=vis,
                                                 ...
                                                 refant=selfcal_library[vis]['refant'], calmode=selfcal_plan['solmode'][
                                                     iteration], solnorm=solnorm if applymode == "calflag" else False,
                                                 ...
                                                 field=incl_targets, scan=incl_scans, gaintable=gaincal_preapply_gaintable, spwmap=gaincal_spwmap, uvrange=selfcal_library[
                                                     'uvrange'],
        ...
        for fid in np.intersect1d(selfcal_library['sub-fields-to-selfcal'], list(selfcal_library['sub-fields-fid_map'][vis].keys())):
            gaincal_spwmap = []
            gaincal_preapply_gaintable = selfcal_library[fid][vis][selfcal_library[fid]
                                                                   ['final_phase_solint']]['gaintable']
            gaincal_interpolate = [selfcal_library['applycal_interp']]*len(gaincal_preapply_gaintable)
            gaincal_gaintype = 'T' if applymode == "calflag" or second_iter_solmode == "" else "GSPLINE" if second_iter_solmode == "GSPLINE" else "G"
            gaincal_solmode = "" if applymode == "calflag" or second_iter_solmode == "GSPLINE" else second_iter_solmode
            if 'spw' in selfcal_plan[vis]['inf_EB_gaincal_combine']:
                applycal_spwmap = [selfcal_library[fid][vis]['spwmap'], selfcal_library[fid]
                                   [vis]['spwmap'], selfcal_library[fid][vis]['spwmap']]
                gaincal_spwmap = [selfcal_library[fid][vis]['spwmap'], selfcal_library[fid][vis]['spwmap']]
            elif selfcal_plan[vis]['inf_EB_fallback_mode'] == 'spwmap':
                applycal_spwmap = selfcal_library[fid][vis]['inf_EB']['spwmap'] + \
                    [selfcal_library[fid][vis]['spwmap'], selfcal_library[fid][vis]['spwmap']]
            ...
            applycal_gaintable = selfcal_library[fid][vis][selfcal_library[fid]['final_phase_solint']
                                                           ]['gaintable']+[sani_target+'_'+vis+'_'+band+'_'+solint+'_'+str(iteration)+'_ap.g']
        ...
        subt.copy(sani_target+'_'+vis+'_'+band+'_'+solint+'_'+str(iteration) +
                  '_'+selfcal_plan['solmode'][iteration]+'.g', deep=True)
        subt.close()

        os.system("rm -rf temp.g")

    if rerank_refants:
        selfcal_library[vis]["refant"] = rank_refants(
            vis, caltable=sani_target+'_'+vis+'_'+band+'_'+solint+'_'+str(iteration)+'_'+selfcal_plan['solmode'][iteration]+'.g')
            ...
            for it, sint in enumerate(selfcal_plan['solints'][0:iteration+1]):
                ...
                if os.path.exists(sani_target+'_'+vis+'_'+band+'_'+sint+'_'+str(it)+'_'+selfcal_plan['solmode'][it]+'.pre-pass.g'):
                    ...
                    os.system("rm -rf "+sani_target+'_'+vis+'_'+band+'_'+sint +
                              '_'+str(it)+'_'+selfcal_plan['solmode'][it]+'.g')
                    ...
                    unflag_failed_antennas(vis, sani_target+'_'+vis+'_'+band+'_'+sint+'_'+str(it)+'_' +
                                           selfcal_plan['solmode'][it] +
                                           '.g', selfcal_library[vis][sint]['gaincal_return'],
    ...
    if 'inf_EB' in solint and fallback == '':
        ...
        for gaintype in np.unique([gaincal_gaintype, 'T']):
            ...
            for i in range(selfcal_library['spws_set'][vis].shape[0]):
                if nspw_sets == 1 and selfcal_library['spws_set'][vis].ndim == 1:
                    spwselect = ','.join(str(spw) for spw in selfcal_library['spws_set'][vis].tolist())
                else:
                    spwselect = ','.join(str(spw) for spw in selfcal_library['spws_set'][vis][i].tolist())
        ...
        fallback, map_index, spwmap, applycal_spwmap_inf_EB = analyze_inf_EB_flagging(selfcal_library, band, spwlist, sani_target+'_'+vis+'_'+band+'_'+solint+'_'+str(
            iteration)+'_'+selfcal_plan['solmode'][iteration]+'.g', vis, target, 'test_inf_EB_'+gaincal_gaintype+'.g', selfcal_library['spectral_scan'], telescope, selfcal_plan['solint_snr_per_spw'], minsnr_to_proceed, 'test_inf_EB_T.g' if gaincal_gaintype == 'G' else None)
            ...
            if 'combinespw' in fallback:
                ...
                for gaintype in np.unique([gaincal_gaintype, 'T']):
                    os.system('cp -r test_inf_EB_'+gaintype+'.g '+sani_target+'_'+vis+'_'+band+'_'+solint +
                              '_'+str(iteration)+'_'+selfcal_plan['solmode'][iteration]+'.gaintype'+gaintype+'.g')
                if fallback == 'combinespw':
                    gaincal_gaintype = 'G'
                else:
                    gaincal_gaintype = 'T'
                os.system('mv test_inf_EB_'+gaincal_gaintype+'.g '+sani_target+'_'+vis+'_'+band +
                          '_'+solint+'_'+str(iteration)+'_'+selfcal_plan['solmode'][iteration]+'.g')
    ...
    if selfcal_library['obstype'] == 'mosaic' or mode == "cocal":
        ...
        tb.open(sani_target+'_'+vis+'_'+band+'_'+solint+'_'+str(iteration) +
                '_'+selfcal_plan['solmode'][iteration]+'.g', nomodify=False)
